...
haCol_protein_meta <- columnAnnotation(
  Batch = protein_meta$Batch,
  `Reproductive Status` = protein_meta$ReproductiveStatus,
  `Race` = protein_meta$Race,
  `Age` = protein_meta$`Age at time of surgery`,
  `Pregnancy` = protein_meta$Pregnancy,
  `Days since LMP` = protein_meta$DaysSinceLMP_use,
  `BRCA status` = protein_meta$groupBRCA,
  `MIR200C` = protein_meta$MIR200cAvgBeta,
  col = list(
...
protein_meta_BYAGE <- dplyr::arrange(protein_meta,`Age at time of surgery`)
haCol_protein_meta_by_AGE <- columnAnnotation(
  Batch = protein_meta_BYAGE$Batch,
  `Reproductive Status` = protein_meta_BYAGE$ReproductiveStatus,
  `Race` = protein_meta_BYAGE$Race,
  `Age` = protein_meta_BYAGE$`Age at time of surgery`,
  `Pregnancy` = protein_meta_BYAGE$Pregnancy,
  `Days since LMP` = protein_meta_BYAGE$DaysSinceLMP_use,
  `BRCA status` = protein_meta_BYAGE$groupBRCA,
  `MIR200C` = protein_meta_BYAGE$MIR200cAvgBeta,
  col = list(
    `MIR200C` = circlize::colorRamp2(
                              breaks = seq(from = 0, to = 1, length = 20),
                              colors = viridis::cividis(20)
...
grep('RNLS',rownames(protein_data)) # no
...
x <- data.frame(
  ...
  `HCK_Expression` = protein_data[grep('HCK',rownames(protein_data)),], check.names=FALSE
)
...
dplyr::filter(x,!is.na(group)) %>%
  ggplot(aes(x=group,y=HCK_Expression)) + 
  # geom_boxplot() +
  geom_violin(fill=NA,draw_quantiles = c(0.25, 0.5, 0.75)) +
  geom_jitter(size = 2, alpha = 1, width = 0.2,aes(col=ReproductiveStatus)) +
  theme_minimal() +
  scale_color_manual(values=c("#28BBECFF","#FB8022FF")) + theme(legend.position = 'none') 
...
protein_meta$cluster2 <- ifelse(
  protein_meta$cluster=='Cluster 1 high sec','Luteal',
  ifelse(protein_meta$cluster=='Cluster 2 high pro','Follicular',
  ifelse(protein_meta$ReproductiveStatus=='Post','Postmeno',
         ifelse(protein_meta$Pregnancy=='Pregnant','Postpartum','Inactive')))
)

table(protein_meta$cluster2)
library(SingleCellExperiment)
library(scran)
protein_data_noNA <- protein_data
protein_data_noNA[is.na(protein_data_noNA)] <- 0 
sce <- SingleCellExperiment(assays = list(counts=protein_data_noNA), colData=protein_meta)

protein.markers <- findMarkers(sce, pval.type="all", direction="up",groups = colData(sce)$cluster2,assay.type='counts',test.type="t",min.prop=0.5)
...
for(i in 1:length(protein.markers)){
  tmp <- data.frame(protein.markers[[i]]) %>% dplyr::arrange(p.value)
  pm <- rbind(
    pm,
    data.frame(
      marker = rep(names(protein.markers)[i],n),
      protein = rownames(tmp)[1:n]
    )
  )
}
...
for(i in 1:length(protein.markers)){
  tmp <- data.frame(protein.markers[[i]]) %>% dplyr::arrange(p.value) %>% dplyr::filter(p.value<0.05)
...
all_markers <- tibble::rownames_to_column(all_markers,'protein')
...
scaled_mat = t(scale(t(protein_data[pm$protein,meta00$`Patient ID`]))); dim(scaled_mat)
# change to patientID
...
pm$marker = factor(pm$marker,levels=levels(meta00$cluster2))
#++++++++++++++++++++++++
#+
## get anno
haCol_rna_markers <- HeatmapAnnotation(
...
hmDEP
pdf('protein_5_RNA_groups_findMarkers_heatmap.pdf',width=12,height=7.5); print(hmDEP); dev.off()
