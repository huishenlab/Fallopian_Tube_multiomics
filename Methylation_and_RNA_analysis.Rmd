...
I did this with the R-4.1.2 module in the collab (i.e. globus) directory on hpc
Did this again on 9/29/23 with the pilot D samples only -- did not reupload the others

```{r upload_Meth_bedfiles_2_synapse,eval=FALSE}
# getwd()
install.packages("synapserutils", repos=c("http://ran.synapse.org", "http://cran.fhcrc.org"))
library(synapserutils)
synLogin(email = 'ian.beddows@vai.org',password = 'Iamquiteconfused.You2?',rememberMe = TRUE)

# gray foundation bulkRNAseq is synapse: syn25813068
# list.files()
# upload a test
# copy(entity = "batch1/SHEH_20191203_FFPEtest_RNA/SVC001370_0006_L000_R1_001.fastq.gz", destinationId = "syn25813068")

# manifest <- data.frame(
  # path = "batch1/SHEH_20191203_FFPEtest_RNA/SVC001370_0006_L000_R1_001.fastq.gz",
  # path = "synapse_upload_1.file",
  # parent = 'syn25813068' # this is the Bulk RNAseq for Gray Foundation
  # parent = 'syn26204933'
# )

# write.table(manifest,file='test_manifest2.tsv',sep="\t",quote=FALSE,row.names = FALSE)


# need to module load python3 first!
# syncToSynapse(manifestFile = 'synapse_manifest_Meth_lvl3.tsv',dryRun=FALSE,sendMessages=TRUE)

# Pilot C RNA:
# syncToSynapse(manifestFile = 'synapse_manifest_RNA_N54.tsv',dryRun=TRUE,sendMessages=TRUE)

# Pilot C Meth:
# on HPC with module R-4.2.1
# > getwd()
# [1] "/varidata/researchtemp/hpctmp/ian.beddows/canary_WGBS_snakemake/Biscuit_Snakemake_Workflow/analysis/pileup"
syncToSynapse(manifestFile = '../',dryRun=FALSE,sendMessages=TRUE)
```
